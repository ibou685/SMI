// frontend/src/pages/Rapports/RapportsPage.jsx
// Page complète de rapports avancés avec filtres, exports PDF/Excel
...
export function RapportsPage() {
  ...
  // Filtres
  const [typeRapport, setTypeRapport] = useState('complet'); // complet, filiale, domaine, gerants, budget
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [filialesRes, domainesRes, recettesRes, depensesRes, alertesRes] = await Promise.all([
  ...
  alert('🚀 BOUTON CLIQUÉ! typeRapport=' + typeRapport + ', filiale=' + filialeSelectionnee);
    console.log('📅 DATES:', { dateDebut, dateFin });  
    console.log('🚀 GENERATERAPPORT APPELÉE!');
    console.log('État AVANT params:', { 
    filialeSelectionnee,
    typeRapport,
    dateDebut,
    dateFin 
    ...
    try {
      ...
      console.log('🔍 Filtres selectionnés:');
      console.log('  - dateDebut:', dateDebut);
      console.log('  - dateFin:', dateFin);
      console.log('  - typeRapport:', typeRapport);
      console.log('  - filialeId:', filialeSelectionnee);
  ...
  case 'filiale':
    if (!filialeSelectionnee) {
      alert('❌ Sélectionnez une filiale');
      return;
    }
  ...
  const exportExcel = async () => {
    try {
      ...
      alert('✅ Rapport Excel généré et téléchargé');
    } catch (err) {
      alert(`❌ Erreur: ${err.message}`);
    } finally {
      setLoading(false);
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-4 pt-4 border-t-2">
              <button
                ...
                disabled={loading}
                className="flex-1 px-6 py-3 bg-red-700 text-white font-bold rounded-lg hover:bg-red-800 transition disabled:bg-gray-400"
              >
              ...
              <button
                onClick={exportExcel}
                disabled={loading}
                className="flex-1 px-6 py-3 bg-green-700 text-white font-bold rounded-lg hover:bg-green-800 transition disabled:bg-gray-400"
              >
